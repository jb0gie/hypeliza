...
export class HyperfyService extends Service {
	...
	async connect(config: { wsUrl: string; authToken?: string; worldId: UUID }): Promise<void> {
		...
		try {
			...
			// 		console.info('[PuppeteerManager] Initialized successfully after delay');
			// 	} catch (error) {
			// 		console.error('[PuppeteerManager] Failed to initialize:', error);
			// 	}
			// }, 45000); // Initialize after 45 seconds to let system stabilize
			await new Promise(resolve => setTimeout(resolve, 100));
			...
			// Start voice manager with delay
			await new Promise(resolve => setTimeout(resolve, 500));
	...
	private async waitForPlayerAndStartBehavior(): Promise<void> {
		...
		while (elapsed < maxWaitTime) {
			if (this.world?.entities?.player) {
				...
				// Reduced delay before starting behavior manager (was 5000ms)
