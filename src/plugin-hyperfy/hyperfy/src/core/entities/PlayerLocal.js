...
export class PlayerLocal extends Entity {
  ...
  async init() {
    ...
    // Add debug logging for physics initialization
    if (typeof PHYSX === 'undefined') {
      console.error('[PlayerLocal] PHYSX is not defined! Physics will not work.');
    } else {
      console.log('[PlayerLocal] Creating ground sweep geometry with PHYSX');
    }
  ...
  fixedUpdate(delta) {
    ...
    if (anchor) {
      ...
      let sweepHit
      {
        ...
        if (!this.groundDebugLogged) {
          console.log('[PlayerLocal] Ground sweep params:', {
            origin: `${origin.x.toFixed(2)}, ${origin.y.toFixed(2)}, ${origin.z.toFixed(2)}`,
            direction: 'DOWN',
            maxDistance,
            hitMask,
            hasPhysics: !!this.world.physics,
            hasSweep: !!this.world.physics?.sweep
          });
