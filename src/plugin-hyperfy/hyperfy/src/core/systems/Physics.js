...
export class Physics extends System {
  ...
  async init() {
    ...
    // Use CPU broad phase to avoid thread issues
  ...
  sweep(geometry, origin, direction, maxDistance, layerMask) {
    // If extensions aren't on the passed vectors, they were created before PhysX init
    // We need to manually add the methods
    if (!origin.toPxVec3) {
      if (THREE.Vector3.prototype.toPxVec3) {
        // Copy the method from the prototype
        origin.toPxVec3 = THREE.Vector3.prototype.toPxVec3;
        origin.fromPxVec3 = THREE.Vector3.prototype.fromPxVec3;
        origin.toPxExtVec3 = THREE.Vector3.prototype.toPxExtVec3;
        origin.toPxTransform = THREE.Vector3.prototype.toPxTransform;
      } else {
        // PhysX not ready yet, return null (no hit)
        return null;
      }
    }
    
    if (!direction.toPxVec3) {
      if (THREE.Vector3.prototype.toPxVec3) {
        direction.toPxVec3 = THREE.Vector3.prototype.toPxVec3;
        direction.fromPxVec3 = THREE.Vector3.prototype.fromPxVec3;
        direction.toPxExtVec3 = THREE.Vector3.prototype.toPxExtVec3;
        direction.toPxTransform = THREE.Vector3.prototype.toPxTransform;
      } else {
        // PhysX not ready yet, return null (no hit)
        return null;
      }
  ...
  overlapSphere(radius, origin, layerMask) {
    // Check if PhysX extensions are available
    if (!origin.toPxVec3) {
      if (THREE.Vector3.prototype.toPxVec3) {
        // Copy the method from the prototype
        origin.toPxVec3 = THREE.Vector3.prototype.toPxVec3;
        ...
        // PhysX not ready yet, return empty array
        return [];
      }
    }
