...
export class ClientNetwork extends System {
  ...
  init({ wsUrl, name, avatar, initialAuthToken }) {
    // Use provided auth token or try to get from storage
    const authToken = initialAuthToken || storage.get('authToken')
    let url = wsUrl
    const params = []
    if (authToken) params.push(`authToken=${authToken}`)
    if (name) params.push(`name=${encodeURIComponent(name)}`)
    if (avatar) params.push(`avatar=${encodeURIComponent(avatar)}`)
    if (params.length > 0) url += '?' + params.join('&')
    
    console.log('[ClientNetwork] Connecting to:', url)
    console.log('[ClientNetwork] Auth token present:', !!authToken)
    console.log('[ClientNetwork] Auth token source:', initialAuthToken ? 'provided' : 'storage')
    
    this.ws = new WebSocket(url)
    this.ws.binaryType = 'arraybuffer'
    
    this.ws.addEventListener('open', () => {
      console.log('[ClientNetwork] WebSocket opened successfully')
    })
    
    this.ws.addEventListener('error', (error) => {
      console.error('[ClientNetwork] WebSocket error:', error)
    })
