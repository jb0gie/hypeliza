      ...
      try {
        ...
        const finalizePhysX = (instance) => {
          if (!instance || typeof instance.PHYSICS_VERSION === 'undefined') {
            reject(new Error('Failed to initialize PhysX instance.'));
            return;
          }
          ...
          resolve({ version, allocator, errorCb, foundation }); // Resolve the outer promise
        };

        // Check for runtime initialization callback
        if (physxInstance && typeof physxInstance.onRuntimeInitialized === 'function') {
          ...
          // IMPORTANT: Do not resolve the outer promise here; wait for the callback
           ...
           console.log('[loadNodePhysX] onRuntimeInitialized not found, proceeding immediately...');
           finalizePhysX(physxInstance); // Call final steps immediately
        }
